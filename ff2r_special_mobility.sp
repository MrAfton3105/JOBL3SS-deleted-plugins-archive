/*
	...
	"special_jump"
    {
        "slot"             "1"                     // Slot umiejętności
        "button"           "11"                    // Przycisk aktywacji (11 = M2)
        "charge"           "1.5"                   // Czas ładowania skoku (w sekundach)
        "cooldown"         "5.0"                  // Cooldown skoku (w sekundach)
        "upward"           "750.0"                // Siła skoku w górę
        "forward"          "1.0"                  // Mnożnik prędkości do przodu
        "emergency"        "2000.0"               // Siła awaryjnego skoku (opcjonalnie)

      
        "strings_charge"   "Super Jump Charging [%.0f%%]" // Komunikat ładowania
        "strings_cooldown" "Super Jump On Cooldown [%.1f]" // Komunikat cooldownu
        "strings"          "Super Jump Ready [Hold M2]"    // Komunikat gotowości

    
        "hud_x"            "-1.0"                  // Pozycja X na HUD (-1.0 = automatycznie)
        "hud_y"            "0.75"                  // Pozycja Y na HUD

   
        "plugin_name"      "ff2r_special_mobility" // Nazwa pluginu
    }
...
bool SJ_Enabled[MAXTF2PLAYERS];
float SJ_Charge[MAXTF2PLAYERS];
float SJ_Cooldown[MAXTF2PLAYERS];
Handle HudSuperJump;
...
public void OnPluginStart()
{
	...
	HudSuperJump = CreateHudSynchronizer();
}
...
public void FF2R_OnBossCreated(int clientIdx, BossData cfg, bool setup)
{
	if(!setup || FF2R_GetGamemodeType() != 2)
	{
        ...
        if (cfg.GetAbility("special_jump").IsMyPlugin())
        {
            SJ_Enabled[clientIdx] = true;
            SJ_Charge[clientIdx] = 0.0;
            SJ_Cooldown[clientIdx] = GetGameTime() + cfg.GetAbility("special_jump").GetFloat("initial", 8.0);
        }
    }
}
...
public void FF2R_OnAbility(int clientIdx, const char[] ability, AbilityData cfg)
{
    ...
    if (!StrContains(ability, "special_jump", false))
    {
        SuperJump(clientIdx, cfg);
    }
}
...
{
    float grv_force = ability.GetFloat("gravity", 6.0); // Pobierz wartość grawitacji z konfiguracji
    float vel_force = ability.GetFloat("downward", -5000.0); // Pobierz wartość prędkości z konfiguracji
    float ang_limit = ability.GetFloat("angle", 60.0); // Pobierz limit kąta z konfiguracji
    ...
    if(GetClientButtons(clientIdx) & IN_DUCK)
    {
        if(!(GetEntityFlags(clientIdx) & FL_ONGROUND))
        {
            ...
            if(ang[0] > ang_limit)
            {
                ...
                // Wyświetl komunikat
                char message[256];
                ability.GetString("strings", message, sizeof(message), "You have used weighdown Test");
                PrintToChat(clientIdx, message);
            }
        }
...
public void OnPlayerRunCmdPost(int clientIdx, int buttons/*, int impulse, const float velocity[3], const float angles[3]*/)
{
	...
	{	
		...
		if (SJ_Enabled[clientIdx])
        {
            Tick_SuperJump(clientIdx, buttons);
        }
	}
}
...
public void SuperJump(int clientIdx, AbilityData ability)
{
    float chargeTime = ability.GetFloat("charge", 1.5); // Czas ładowania skoku
    float cooldownTime = ability.GetFloat("cooldown", 5.0); // Cooldown skoku
    float upwardForce = ability.GetFloat("upward", 750.0); // Siła skoku w górę
    float forwardForce = ability.GetFloat("forward", 1.0); // Mnożnik prędkości do przodu

    // Sprawdź, czy skok jest na cooldownie
    if (SJ_Cooldown[clientIdx] > GetGameTime())
    {
        char cooldownMsg[256];
        ability.GetString("strings_cooldown", cooldownMsg, sizeof(cooldownMsg), "Super Jump On Cooldown [%.1f]");
        PrintToChat(clientIdx, cooldownMsg, SJ_Cooldown[clientIdx] - GetGameTime());
        return;
    }

    // Sprawdź, czy gracz trzyma przycisk M2 (IN_ATTACK2)
    if (GetClientButtons(clientIdx) & IN_ATTACK2)
    {
        // Ładowanie skoku
        if (SJ_Charge[clientIdx] < chargeTime)
        {
            SJ_Charge[clientIdx] += GetGameFrameTime(); // Zwiększ ładunek skoku
            char chargeMsg[256];
            ability.GetString("strings_charge", chargeMsg, sizeof(chargeMsg), "Super Jump Charging [%.0f%%]");
            PrintToChat(clientIdx, chargeMsg, (SJ_Charge[clientIdx] / chargeTime) * 100.0); // Wyświetl postęp ładowania
        }
        else
        {
            // Wykonaj skok
            ...
            // Oblicz siłę skoku
            vel[2] = upwardForce + (SJ_Charge[clientIdx] * 3.25); // Siła w górę
            vel[0] *= forwardForce + (SJ_Charge[clientIdx] * 0.00275); // Siła do przodu (X)
            vel[1] *= forwardForce + (SJ_Charge[clientIdx] * 0.00275); // Siła do przodu (Y)

            // Zastosuj skok
            TeleportEntity(clientIdx, NULL_VECTOR, NULL_VECTOR, vel);

            // Zresetuj ładunek i ustaw cooldown
            SJ_Charge[clientIdx] = 0.0;
            SJ_Cooldown[clientIdx] = GetGameTime() + cooldownTime;

            // Wyświetl komunikat o gotowości
            char readyMsg[256];
            ability.GetString("strings", readyMsg, sizeof(readyMsg), "Super Jump Ready [Hold M2]");
            PrintToChat(clientIdx, readyMsg);
        }
    }
    else if (SJ_Charge[clientIdx] > 0.0)
    {
        // Jeśli gracz puścił przycisk przed zakończeniem ładowania
        SJ_Charge[clientIdx] = 0.0; // Zresetuj ładunek
        char readyMsg[256];
        ability.GetString("strings", readyMsg, sizeof(readyMsg), "Super Jump Ready [Hold M2]");
        PrintToChat(clientIdx, readyMsg);
    }
...
public Action Tick_SuperJump(int clientIdx, int &buttons)
{
    AbilityData ability = FF2R_GetBossData(clientIdx).GetAbility("special_jump");
    ...
    // Wyświetl HUD z informacją o stanie skoku
    if (!(buttons & IN_SCORE) && ability.GetFloat("hudin") < gameTime)
    {
        ability.SetFloat("hudin", gameTime + 0.09);

        float hud_x = ability.GetFloat("hud_x", -1.0);
        float hud_y = ability.GetFloat("hud_y", 0.75);

        char strings[256];
        ability.GetString("strings", strings, sizeof(strings), "Super Jump Ready [Hold M2]");

        if (SJ_Cooldown[clientIdx] > GetGameTime())
        {
            // Skok na cooldownie
            SetHudTextParams(hud_x, hud_y, 0.1, 255, 64, 64, 255);
            ShowSyncHudText(clientIdx, HudSuperJump, "Super Jump On Cooldown [%.1f]", SJ_Cooldown[clientIdx] - GetGameTime());
        }
        else
        {
            // Skok gotowy
            SetHudTextParams(hud_x, hud_y, 0.1, 255, 255, 255, 255);
            ShowSyncHudText(clientIdx, HudSuperJump, strings);
        }
    }
